#!/usr/bin/env python3
"""第十八轮高难度回归（15 个）：结构镜像已验证模式，目标首次运行全绿零修改。
用法: python3 tests/run_tests18.py
"""
...
def main():
    ...
    # 1. fetch p4 --json
    print("[1/18] fetch p4 --json")
    p = run_cli(["fetch", f"{base}/p4.html", "--json"])
    try:
        data = json.loads(p.stdout)
        ok = data.get("status") == 200 and "p4-title-1" in data.get("text", "")
    except Exception:
        ok = False
    check("q1_fetch_p4", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch htmlp1 --json
    print("[2/18] fetch htmlp1 --json")
    p = run_cli(["fetch", f"{base}/htmlp1.html", "--json"])
    try:
        data = json.loads(p.stdout)
        ok = data.get("status") == 200 and "hp-1" in data.get("text", "")
    except Exception:
        ok = False
    check("q2_htmlp1", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 3. v3 p3 双字段
    print("[3/18] v3 p3 双字段")
    name = "q3"
    cfg = scfg(name, [f"{base}/p3.html"])
    ...
    # 4. v3 五页并发 8
    print("[4/18] v3 五页并发 8")
    name = "q4"
    d = make_task(name, scfg(name, [f"{base}/p{i}.html" for i in range(1, 6)], concurrency=8))
    try:
        r = run_task(d)
        urls = {i["_url"] for i in read_items(name)}
        check(name, r["total"] == 10 and r["fetched"] == 5 and len(urls) == 5,
              f"total={r['total']} fetched={r['fetched']}")
    finally:
        cleanup(name)

    # 5. json_paged list
    print("[5/18] json_paged list")
    name = "q5"
    ...
    try:
        r = run_task(d)
        urls = [i["_url"] for i in read_items(name)]
        ok = r["fetched"] == 3 and not any("p2.html" in u for u in urls)
        check(name, ok, f"fetched={r['fetched']} urls={sorted(set(urls))}")
    finally:
        cleanup(name)

    # 8. multi jsonl+sqlite p3
    print("[8/18] multi jsonl+sqlite p3")
    name = "q8"
    cfg = scfg(name, [f"{base}/p3.html"],
    ...
    try:
        ...
        rows = read_json(name)
        check(name, r2["total"] == 4 and len(rows) == 6, f"r2={r2['total']} rows={len(rows)}")
    finally:
        cleanup(name)

    # 10. crawl --max 1
    print("[10/18] crawl --max 1")
    ...
    for f_ in glob.glob(str(ROOT / "outputs" / "crawl_127_0_0_1_*")):
        try:
            ...
            pass
    check("q10_crawl1", len(urls) == 1 and "hub.html" in urls[0], f"urls={urls}")

    # 11. fetch --browser --selector spa
    print("[11/18] fetch --browser --selector spa")
    p = run_cli(["fetch", f"{base}/spa.html", "--browser", "--selector", ".item"], timeout=300)
    ok = "spa-item-1" in p.stdout and "spa-item-3" in p.stdout
    check("q11_browser_sel", ok, f"stdout={p.stdout[:80]}")

    # 12. jobs 3 好任务
    print("[12/18] jobs 3 好任务")
    names = ["q12a", "q12b", "q12c"]
    for nm in names:
        make_task(nm, scfg(nm, [f"{base}/p1.html"]))
    jobs_fp = ROOT / "outputs" / ".test_jobs18.json"
    ...
    try:
        ...
        check("q12_jobs", data.get("jobs") == 3 and data.get("total_items") == 6,
              f"data={data}")
    finally:
    ...
    # 13. monitor shrink 消失
    print("[13/18] monitor shrink 消失")
    name = "q13"
    ...
    d = make_task(name, cfg)
    try:
        r = run_task(d, overrides={"q": "广州"})
        items = read_items(name)
        check(name, r["total"] == 1 and items[0]["title"] == "hit-广州",
              f"total={r['total']} items={[i['title'] for i in items]}")
    finally:
        cleanup(name)

    # 15. fetch --screenshot article
    print("[15/18] fetch --screenshot article")
    shot = "/tmp/us_shot18.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("q15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
