...
import importlib
MODULE = os.environ.get("CHALLENGE_MOD", "challenges")
server_mod = importlib.import_module(MODULE + ".server")
make_server = server_mod.make_server
CHALLENGES = server_mod.CHALLENGES
HOST, PORT = server_mod.HOST, server_mod.PORT
S = importlib.import_module(MODULE + ".solutions")
...
def run(only=None, only_fail=False, verbose=False):
    ...
    try:
        ...
        srv.shutdown()
    out = ROOT / "challenges2" / "results.json"
