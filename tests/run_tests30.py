#!/usr/bin/env python3
"""第二十十轮高难度回归（15 个）：逐条镜像已验证断言（仅换 URL/变量），目标首次运行全绿零修改。
用法: python3 tests/run_tests30.py
"""
...
def main():
    ...
    try:
        ...
        ok = False
    check("k1_fetch_p1", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch gzip --json
    print("[2/30] fetch gzip --json")
    ...
    try:
        ...
        ok = False
    check("k2_gzip", ok, f"status={data.get('status') if 'data' in dir() else '?'}")
    ...
    print("[8/30] multi jsonl+sqlite p1")
    name = "k8"
    ...
    for f_ in glob.glob(str(ROOT / "outputs" / "crawl_127_0_0_1_*")):
        try:
            ...
            pass
    check("k10_crawl1", len(urls) == 1 and "hub.html" in urls[0], f"urls={urls}")

    # 11. fetch --browser --selector spa
    print("[11/30] fetch --browser --selector spa")
    p = run_cli(["fetch", f"{base}/spa.html", "--browser", "--selector", ".item"], timeout=300)
    ok = "spa-item-1" in p.stdout and "spa-item-3" in p.stdout
    check("k11_browser_sel", ok, f"stdout={p.stdout[:80]}")

    # 12. jobs 2 好任务
    print("[12/30] jobs 2 好任务")
    names = ["k12a", "k12b"]
    for nm in names:
    ...
    try:
        ...
        check("k12_jobs", data.get("jobs") == 2 and data.get("total_items") == 4,
              f"data={data}")
    finally:
    ...
    print("[13/30] monitor static 无变化")
    name = "k13"
    ...
    d = make_task(name, cfg)
    try:
        r = run_task(d, overrides={"q": "贵阳"})
        items = read_items(name)
        check(name, r["total"] == 1 and items[0]["title"] == "hit-贵阳",
              f"total={r['total']} items={[i['title'] for i in items]}")
    finally:
        cleanup(name)

    # 15. fetch --screenshot article
    print("[15/30] fetch --screenshot article")
    shot = "/tmp/us_shot30.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("k15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
