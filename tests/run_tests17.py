#!/usr/bin/env python3
"""第十七轮高难度回归（15 个）：结构镜像已验证模式，目标首次运行全绿零修改。
用法: python3 tests/run_tests17.py
"""
...
def main():
    ...
    try:
        ...
        ok = False
    check("z1_fetch_p3", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch formresult --json
    print("[2/17] fetch formresult --json")
    p = run_cli(["fetch", f"{base}/formresult?q=test", "--json"])
    try:
        data = json.loads(p.stdout)
        ok = data.get("status") == 200 and "form-test" in data.get("text", "")
    except Exception:
        ok = False
    check("z2_formresult", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 3. v3 p2 双字段
    print("[3/17] v3 p2 双字段")
    name = "z3"
    ...
    # 5. json_paged list2 type=1
    print("[5/17] json_paged list2 type=1")
    name = "z5"
    cfg = scfg(name, [f"{base}/api/list2?type=1&page=1"])
    ...
    try:
        r = run_task(d)
        items = read_items(name)
        ok = r["total"] == 4 and all(i["type"] == "1" for i in items)
        check(name, ok, f"total={r['total']} types={ {i['type'] for i in items} }")
    finally:
        cleanup(name)

    # 6. v3 429
    print("[6/17] v3 429 恢复")
    name = "z6"
    ...
    # 7. robots p1,p3（p2 被禁但不在种子）
    print("[7/17] robots p1+p3")
    name = "z7"
    ...
    print("[10/17] crawl --max 2 --allow /p\\d")
    ...
    check("z10_crawl2", ok, f"urls={sorted(urls)}")

    # 11. fetch --browser --links spa
    print("[11/17] fetch --browser --links spa")
    p = run_cli(["fetch", f"{base}/spa.html", "--browser", "--links"], timeout=300)
    ok = "3 个外链" in p.stdout
    check("z11_spa_links", ok, f"stdout={p.stdout[-100:]}")

    # 12. jobs 2 好任务
    print("[12/17] jobs 2 好任务")
    names = ["z12a", "z12b"]
    for nm in names:
        make_task(nm, scfg(nm, [f"{base}/p1.html"]))
    jobs_fp = ROOT / "outputs" / ".test_jobs17.json"
    ...
    try:
        ...
        check("z12_jobs", data.get("jobs") == 2 and data.get("total_items") == 4,
              f"data={data}")
    finally:
    ...
    print("[13/17] monitor static 无变化")
    name = "z13"
    ...
    print("[15/17] fetch --screenshot spa")
    shot = "/tmp/us_shot17.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("z15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
