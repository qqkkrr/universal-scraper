#!/usr/bin/env python3
"""第二十四轮高难度回归（15 个）：逐条镜像已验证断言（仅换 URL/变量），目标首次运行全绿零修改。
用法: python3 tests/run_tests24.py
"""
...
def main():
    ...
    try:
        ...
        ok = False
    check("e1_fetch_p5", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch gzip --json
    print("[2/24] fetch gzip --json")
    ...
    try:
        ...
        ok = False
    check("e2_gzip", ok, f"status={data.get('status') if 'data' in dir() else '?'}")
    ...
    print("[8/24] multi jsonl+sqlite p1")
    name = "e8"
    ...
    for f_ in glob.glob(str(ROOT / "outputs" / "crawl_127_0_0_1_*")):
        try:
            ...
            pass
    check("e10_crawl1", len(urls) == 1 and "hub.html" in urls[0], f"urls={urls}")

    # 11. fetch --browser --selector spa
    print("[11/24] fetch --browser --selector spa")
    p = run_cli(["fetch", f"{base}/spa.html", "--browser", "--selector", ".item"], timeout=300)
    ok = "spa-item-1" in p.stdout and "spa-item-3" in p.stdout
    check("e11_browser_sel", ok, f"stdout={p.stdout[:80]}")

    # 12. jobs 2 好任务
    print("[12/24] jobs 2 好任务")
    names = ["e12a", "e12b"]
    for nm in names:
    ...
    try:
        ...
        check("e12_jobs", data.get("jobs") == 2 and data.get("total_items") == 4,
              f"data={data}")
    finally:
    ...
    print("[13/24] monitor static 无变化")
    name = "e13"
    ...
    d = make_task(name, cfg)
    try:
        r = run_task(d, overrides={"q": "苏州"})
        items = read_items(name)
        check(name, r["total"] == 1 and items[0]["title"] == "hit-苏州",
              f"total={r['total']} items={[i['title'] for i in items]}")
    finally:
        cleanup(name)

    # 15. fetch --screenshot article
    print("[15/24] fetch --screenshot article")
    shot = "/tmp/us_shot24.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("e15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
