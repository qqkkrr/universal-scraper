#!/usr/bin/env python3
"""第二十六轮高难度回归（15 个）：逐条镜像已验证断言（仅换 URL/变量），目标首次运行全绿零修改。
用法: python3 tests/run_tests26.py
"""
...
def main():
    ...
    try:
        ...
        ok = False
    check("g1_fetch_p1", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch gzip --json
    print("[2/26] fetch gzip --json")
    ...
    try:
        ...
        ok = False
    check("g2_gzip", ok, f"status={data.get('status') if 'data' in dir() else '?'}")
    ...
    print("[8/26] multi jsonl+sqlite p1")
    name = "g8"
    ...
    for f_ in glob.glob(str(ROOT / "outputs" / "crawl_127_0_0_1_*")):
        try:
            ...
            pass
    check("g10_crawl1", len(urls) == 1 and "hub.html" in urls[0], f"urls={urls}")

    # 11. fetch --browser --selector spa
    print("[11/26] fetch --browser --selector spa")
    p = run_cli(["fetch", f"{base}/spa.html", "--browser", "--selector", ".item"], timeout=300)
    ok = "spa-item-1" in p.stdout and "spa-item-3" in p.stdout
    check("g11_browser_sel", ok, f"stdout={p.stdout[:80]}")

    # 12. jobs 2 好任务
    print("[12/26] jobs 2 好任务")
    names = ["g12a", "g12b"]
    for nm in names:
    ...
    try:
        ...
        check("g12_jobs", data.get("jobs") == 2 and data.get("total_items") == 4,
              f"data={data}")
    finally:
    ...
    print("[13/26] monitor static 无变化")
    name = "g13"
    ...
    d = make_task(name, cfg)
    try:
        r = run_task(d, overrides={"q": "青岛"})
        items = read_items(name)
        check(name, r["total"] == 1 and items[0]["title"] == "hit-青岛",
              f"total={r['total']} items={[i['title'] for i in items]}")
    finally:
        cleanup(name)

    # 15. fetch --screenshot article
    print("[15/26] fetch --screenshot article")
    shot = "/tmp/us_shot26.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("g15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
