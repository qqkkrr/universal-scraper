#!/usr/bin/env python3
"""第二十一轮高难度回归（15 个）：逐条镜像已验证断言（仅换 URL/变量），目标首次运行全绿零修改。
用法: python3 tests/run_tests21.py
"""
...
def main():
    ...
    try:
        ...
        ok = False
    check("b1_fetch_p1", ok, f"status={data.get('status') if 'data' in dir() else '?'}")

    # 2. fetch gzip --json
    print("[2/21] fetch gzip --json")
    ...
    try:
        ...
        ok = False
    check("b2_gzip", ok, f"status={data.get('status') if 'data' in dir() else '?'}")
    ...
    print("[8/21] multi jsonl+sqlite p1")
    name = "b8"
    ...
    for f_ in glob.glob(str(ROOT / "outputs" / "crawl_127_0_0_1_*")):
        try:
            ...
            pass
    check("b10_crawl1", len(urls) == 1 and "hub.html" in urls[0], f"urls={urls}")
    ...
    try:
        ...
        check("b12_jobs", data.get("jobs") == 2 and data.get("total_items") == 4,
              f"data={data}")
    finally:
    ...
    print("[13/21] monitor static 无变化")
    name = "b13"
    ...
    d = make_task(name, cfg)
    try:
        r = run_task(d, overrides={"q": "成都"})
        items = read_items(name)
        check(name, r["total"] == 1 and items[0]["title"] == "hit-成都",
              f"total={r['total']} items={[i['title'] for i in items]}")
    finally:
        cleanup(name)

    # 15. fetch --screenshot article
    print("[15/21] fetch --screenshot article")
    shot = "/tmp/us_shot21.png"
    try:
        os.remove(shot)
    except FileNotFoundError:
        pass
    try:
        ...
        check("b15_screenshot", ok, f"size={Path(shot).stat().st_size if Path(shot).exists() else 0}")
    finally:
        try:
            os.remove(shot)
        except FileNotFoundError:
