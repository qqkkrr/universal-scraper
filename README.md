...
## 第三十九轮：第七轮 review 修复批次（配置输入健壮性 / 边界防护）

- **#26 浏览器桥非法正则崩溃（P1，复现）**：`capture[].url_pattern` / `login.require_cookie` 来自 LLM 配置，
  非法正则（如 `(`）会让 Node 桥整进程崩溃（exit=1、零输出）。已全部改 try/catch 安全构造
  （非法视为不匹配 + 一次性提示），复现从崩溃 → exit 0 + 正常产出 page。
- **#28 http_4xx 纳入诊断预算**：4xx 响应提示不再刷爆协议流（每类 50 条 + 截断提示）。
- **dianping 空 HTML 防护**：`parse_search_html("")` 返回 [] 不再 lxml 崩溃。
- **verify 字段并集**：字段完整率统计前 200 行的字段并集，不再漏掉后续行新增字段。
- **进程生命周期核查**：实测浏览器池空闲退出/强制终止均**不产生孤儿 Chromium**（原假设证伪，
  未改动；建议跨平台再验一次）。
...
## 第四十二轮：Boss直聘实测强化（一句话任务直接可用）

本轮用 Boss直聘「产品经理岗位」实测暴露 4 个通用短板，已全部修复并全量回归：

- **429 带 Retry-After（秒/HTTP-date）**：429 不再客户端内部空等，立即交还引擎按服务端要求精确调度；HTTP-date 格式（`Retry-After: Fri, 07 Aug 2026 ... GMT`）也能正确解析。403/5xx 重试耗尽后保留真实状态码（不再丢成 0 导致 `HTTP 0` 崩溃）。
- **永久性 4xx 跳过**：404/410 等客户端错误重试无意义 → 新增 `PermanentFetchError`，引擎跳过、不计错误、不重试；`http_error` 不再误判为反爬拦截（之前会把普通 404 当验证码/拦截反复重试）。完成消息显示 `跳过 N`，结果含 `skipped` 统计。
- **选择器自愈**：`row_css` 命中 0 个时自动找页面里重复最多的 `li/div/tr` 链接块做通用提取（上限 200 条）——结构奇怪、选择器写不对的列表也能出数据。
- **浏览器任务默认滚动**：未显式配置时自动滚动 4 次 × 800ms，懒加载/瀑布流站不再 0 条。
- **LLM 兜底改用渲染后页面**：0 条时用引擎保存的 `last_page.html`（登录/JS 页必须用渲染结果，裸 HTML 只会拿到验证页）。
- **日期过滤不再静默全丢**：`between` 遇到日期缺失/无法解析时保留该条并统计（`流水线统计: filter:日期缺失=N`），只有明确区间外的才丢弃。
